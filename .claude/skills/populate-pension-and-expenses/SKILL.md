...
Then locate all five tables in that SAME dump before moving on:

```bash
grep -n -i "CHANGES IN.*NET PENSION\|SCHEDULE OF.*CONTRIBUTIONS\|Changes in Net Position\|Changes in Fund Balances" {scratch}/{id}-fy{year}.txt
```
...
**Per-year statements are only the FALLBACK + cross-check** for the expense half (a year outside
every statistical window, or a table that collapses categories). Locate fast:
`grep -n "Functions/Programs"` → government-wide SoA; `grep -n "EXPENDITURES:"` → governmental-funds
statement (rightmost **Total Governmental Funds** column). Verify a couple of statistical totals
against these. Only then does a third PDF become justified — say so when you report.
...
printed percentage for **every plan-year**. This is the check that catches a misaligned continuation
page or an off-by-one year mapping. If any year disagrees, the parse is wrong — fix it, don't round.

## 5. Write, format, verify — one edit, one build
- Edit ONLY `data/acfr-json/{id}.ts`, adding BOTH halves in the same pass. Field order per
  ...
  the capital fields. Same order → clean diffs and consistency across cities.
- Write plain integers, no `$`/commas; the repo's `1_234_567` numeric separators are fine. Use each
  ACFR's exact dollars (these fields are unrounded, unlike the thousands-rounded base fields).
- Re-run BOTH audits → expect no GAPS (or only genuinely-unsourceable years, which you report),
  `reconciliation ✓`, plausible funded ratios, no anomalies.
- `npx prettier --write data/acfr-json/{id}.ts` (and `lib/expense-category-groups.ts` if edited),
  confirm `--check`.
- `npx tsc --noEmit` (pre-existing errors in `city-map.tsx` / `ui/chart.tsx` are unrelated — grep the
  output for `{id}` / `acfr-json` to confirm none are yours), then `npm run build`. Run this ONCE,
  after both halves are written.
